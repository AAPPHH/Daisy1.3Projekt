...
class Blocker(Player):
    ...
    def blocking(self, game, board):
        ...
        """
        enemy_positions = list(zip(*np.where(board.board == 1)))
        if enemy_positions:
            ...
            if 0 <= new_row < board.m and 0 <= new_col < board.n and board.board[new_row][new_col] == 0:
                print("Blockiert! :-)")
                return Player.place_piece(self, new_row, new_col, game, board)
            else:
